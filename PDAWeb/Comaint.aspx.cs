    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
        {
            ...
            if (Session["piid"].ToString().Trim() != "" && Session["plant"].ToString().Trim() != "" && Session["type"].ToString().Trim() != "" && Session["pi_wec_ctn"].ToString().Trim() != "")
            {
                ...
                _pi_det_model = new PI.DAL.pi_det().GetModel(Session["piid"].ToString(), Session["pi_wec_ctn"].ToString());
                if (_pi_det_model != null)
                {
                    var plr_mstr_tran = new PIE.DAL.plr_mstr_tran().GetModel(_pi_det_model.plr_LineID_tran, _pi_det_model.pi_wec_ctn);
